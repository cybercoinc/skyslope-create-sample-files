...
const getChecklistTypeId = async (sessionToken, officeGuid) => {
  try {
    ...
    const checklistTypeIds = await axios.get(
      `${apiUrl}/api/offices/${officeGuid}/checklisttypes?transactionType=Listing`,
      { headers: { Session: sessionToken } }
    );
...
const main = async () => {
  ...
  const body = {
    mlsNumber: '123456',
    officeGuid: null, // depending on user type (i.e. Broke, Agent), specify office.
    agentGuid: `${userGuid}`,
    checklistTypeId: `${checklistTypeId}`,
    sourceId: 44,
    listingDate: '2018-10-24T18:41:43.020Z',
    property: {
      streetNumber: `${Math.floor(Math.random() * Math.max(99999))}`,
      streetAddress: 'Testing St.',
      unit: '',
      direction: 'N',
      city: 'Sao Paolo',
      county: 'This County',
    ...
    expirationDate: '2018-10-24T18:41:43.020Z',
    listingPrice: 250000,
    commercialLease: {
      size: '15',
      sizeType: 'SF',
  ...
  axios
    .post(`${apiUrl}/api/files/listings`, JSON.stringify(body), config)
    .then((response) => {
